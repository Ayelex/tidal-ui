...
class LosslessAPI {
	...
	private streamDataCache = new Map<string, StreamCacheEntry>();
	private streamDataTtlMs = 3 * 60 * 1000;
	private streamDataPersistentTtlMs = 10 * 24 * 60 * 60 * 1000;
	private streamDataCacheHydrated = false;
	private streamDataStorageKey = 'tidal-ui.stream-cache.v1';
	...
	private buildProxyUrl(target: string): string | null {
		if (!API_CONFIG.useProxy || !API_CONFIG.proxyUrl) {
			return null;
		}
		if (target.startsWith(API_CONFIG.proxyUrl)) {
			return target;
		}
		try {
			const base = typeof window !== 'undefined' ? window.location.origin : undefined;
			const url = base ? new URL(target, base) : new URL(target);
			return `${API_CONFIG.proxyUrl}?url=${encodeURIComponent(url.toString())}`;
		} catch {
			return null;
		}
	}
	...
	private async fetchStreamResponse(
		...
		for (const attemptUrl of attempts) {
			try {
				const response = await fetch(attemptUrl, { signal: options?.signal });
				if (response.status === 429) {
					throw new Error(RATE_LIMIT_ERROR_MESSAGE);
				}
				if (!response.ok) {
					lastError = new Error(`Failed to fetch audio stream (status ${response.status})`);
					continue;
				}
				return response;
			} catch (error) {
	...
	private ensureStreamCacheHydrated(): void {
		if (!browser || this.streamDataCacheHydrated) {
			return;
		}
		this.streamDataCacheHydrated = true;
		try {
			const raw = localStorage.getItem(this.streamDataStorageKey);
			if (!raw) {
				return;
			}
			...
			for (const [key, entry] of Object.entries(parsed)) {
				...
				const lastUsedAt = typeof entry.lastUsedAt === 'number' ? entry.lastUsedAt : entry.fetchedAt;
				if (now - lastUsedAt > this.streamDataPersistentTtlMs) {
					continue;
				}
				this.streamDataCache.set(key, {
					data: entry.data,
					fetchedAt: entry.fetchedAt,
					lastUsedAt
				});
			}
			this.persistStreamCache();
		} catch (error) {
			console.warn('Failed to hydrate stream cache', error);
		}
	}

	private persistStreamCache(): void {
		if (!browser) {
			return;
		}
		try {
			const entries = Array.from(this.streamDataCache.entries());
			const payload: Record<string, StreamCacheEntry> = {};
			for (const [key, entry] of entries) {
				payload[key] = entry;
			}
			localStorage.setItem(this.streamDataStorageKey, JSON.stringify(payload));
		} catch (error) {
			console.warn('Failed to persist stream cache', error);
		}
	}

	private pruneStreamCache(now: number): void {
		const maxAge = this.streamDataPersistentTtlMs;
		let changed = false;
		for (const [key, entry] of this.streamDataCache.entries()) {
			const lastUsedAt =
				typeof entry.lastUsedAt === 'number' ? entry.lastUsedAt : entry.fetchedAt;
			if (now - lastUsedAt > maxAge) {
	...
	private getStreamDataCacheKey(trackId: number, quality: AudioQuality): string {
		return `${trackId}:${quality}`;
	}

	private getCachedStreamData(trackId: number, quality: AudioQuality): ResolvedStreamData | null {
		this.ensureStreamCacheHydrated();
		const key = this.getStreamDataCacheKey(trackId, quality);
		const cached = this.streamDataCache.get(key);
		if (!cached) {
			return null;
		}
		if (Date.now() - cached.fetchedAt > this.streamDataTtlMs) {
			this.streamDataCache.delete(key);
			this.persistStreamCache();
			return null;
		}
		cached.lastUsedAt = Date.now();
		this.streamDataCache.set(key, cached);
		this.persistStreamCache();
		return cached.data;
	}

	private cacheStreamData(trackId: number, quality: AudioQuality, data: ResolvedStreamData): void {
		this.ensureStreamCacheHydrated();
		const key = this.getStreamDataCacheKey(trackId, quality);
		const now = Date.now();
		this.streamDataCache.set(key, { data, fetchedAt: now, lastUsedAt: now });
		this.pruneStreamCache(now);
		this.persistStreamCache();
	}

	invalidateStreamDataCache(trackId: number, quality?: AudioQuality): void {
		this.ensureStreamCacheHydrated();
		if (quality) {
			this.streamDataCache.delete(this.getStreamDataCacheKey(trackId, quality));
			this.persistStreamCache();
			return;
		}
		const prefix = `${trackId}:`;
		for (const key of this.streamDataCache.keys()) {
	...
	private resolveDownloadQuality(quality: AudioQuality): AudioQuality {
		return quality === 'LOSSLESS' ? quality : 'LOSSLESS';
	}
	...
	private async downloadFlacFromMpd(
		...
		for (const url of urls) {
			const response = await this.fetchStreamResponse(url, options);
	...
	async getStreamData(
		...
		const cached = this.getCachedStreamData(trackId, quality);
		if (cached) {
			return cached;
		}
		...
		let effectiveQuality: AudioQuality = quality;
		...
		if (this.isHiResQuality(quality)) {
			try {
				...
				const resolved = { url, replayGain, sampleRate, bitDepth };
				this.cacheStreamData(trackId, effectiveQuality, resolved);
				return resolved;
			} catch (error) {
		...
		for (let attempt = 1; attempt <= 3; attempt += 1) {
			try {
				...
				if (lookup.originalTrackUrl) {
					const resolved = { url: lookup.originalTrackUrl, replayGain, sampleRate, bitDepth };
					this.cacheStreamData(trackId, effectiveQuality, resolved);
					return resolved;
				}
				...
				if (manifestUrl) {
					const resolved = { url: manifestUrl, replayGain, sampleRate, bitDepth };
					this.cacheStreamData(trackId, effectiveQuality, resolved);
					return resolved;
				}
	...
	async fetchTrackBlob(
		...
		const downloadQuality = this.resolveDownloadQuality(quality);
		try {
			const {
				manifestLookup,
				metadataLookup: initialMetadataLookup,
				manifestQuality
			} = await this.resolveTrackLookups(trackId, downloadQuality);
			...
			if (streamUrl) {
				try {
					response = await this.fetchStreamResponse(streamUrl, options);
				} catch (error) {
					if (error instanceof Error && error.message === RATE_LIMIT_ERROR_MESSAGE) {
						throw error;
					}
					console.warn('OriginalTrackUrl download failed, falling back to manifest', error);
					response = null;
				}
			}
			...
			if (!response) {
				...
				if (this.isSegmentedDashManifest(decodedManifest)) {
					...
					if (fallbackUrl) {
						...
						response = await this.fetchStreamResponse(fallbackUrl, options);
						metadataLookup = manifestSource;
					} else {
			...
			const shouldConvertToMp3 =
				options?.convertAacToMp3 === true &&
				(downloadQuality === 'HIGH' || downloadQuality === 'LOW');
	...
	async downloadTrack(
		...
		const downloadQuality = this.resolveDownloadQuality(quality);
		try {
			const { blob } = await this.fetchTrackBlob(trackId, downloadQuality, filename, options);
			...
			if (options?.downloadCoverSeperately) {
				try {
					const metadata = await this.getPreferredTrackMetadata(trackId, downloadQuality);
