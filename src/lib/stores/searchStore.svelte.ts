...
class SearchStore {
	...
	playlistSourceUrl = $state<string | null>(null);
	playlistTitle = $state<string | null>(null);
		...
		if (browser) {
			...
			if (stored) {
				try {
					...
					this.playlistSourceUrl = data.playlistSourceUrl ?? null;
					this.playlistTitle = data.playlistTitle ?? null;
				} catch (e) {
