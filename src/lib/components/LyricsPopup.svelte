<script lang="ts">
	...
	const COMPONENT_ORIGIN = 'https://cdn.jsdelivr.net';
	...
	let prefetchRequested = $state(false);
	...
	$effect(() => {
		if (browser && ($lyricsStore.open || $lyricsStore.track)) {
	...
	$effect(() => {
		if (!browser || prefetchRequested || !$lyricsStore.track) {
			return;
		}
		warmLyricsComponent();
	});
	...
	function scheduleIdle(callback: () => void) {
		...
		if (typeof idle === 'function') {
			...
			prefetchTimer = window.setTimeout(callback, 80);
		}
	}

	function warmLyricsComponent() {
		if (!browser || prefetchRequested) {
			return;
		}
		prefetchRequested = true;
		addHeadLink('preconnect', COMPONENT_ORIGIN, { crossorigin: 'anonymous' });
		addHeadLink('dns-prefetch', COMPONENT_ORIGIN);
		addHeadLink('modulepreload', COMPONENT_MODULE_URL, { crossorigin: 'anonymous' });
		scheduleIdle(() => {
	...
	function handleLineClick(event: Event) {
		...
		playerStore.seekTo(timeSeconds);
		playerStore.play();
	}
...
<div
	class={`lyrics-overlay ${$lyricsStore.open ? 'lyrics-overlay--open' : 'lyrics-overlay--closed'}`}
	role="presentation"
	aria-hidden={!$lyricsStore.open}
...
>
	<div
		...
		role="dialog"
		aria-modal={$lyricsStore.open ? 'true' : undefined}
		aria-label="Lyrics"
	>
					...
					<div class="lyrics-component-wrapper">
						{#key lyricsKey}
							<am-lyrics
								...
								hover-background-color="rgba(239, 68, 68, 0.14)"
...
<style>
	.lyrics-overlay {
		...
		background: linear-gradient(160deg, #0a0608 0%, #14070c 45%, #1c0a12 100%);
	...
	.lyrics-icon-button:not([disabled]):hover {
		border-color: var(--bloom-accent, rgba(239, 68, 68, 0.7));
	...
	.lyrics-retry {
		border: 1px solid var(--bloom-accent, rgba(239, 68, 68, 0.7));
		background: #120a10;
		backdrop-filter: none;
		-webkit-backdrop-filter: none;
		color: #f0f9ff;
	...
	.lyrics-retry:hover {
		...
		box-shadow: inset 0 0 30px rgba(239, 68, 68, 0.18);
	}
