<script lang="ts">
	...
	const animateWebGLBackground = (timestamp: number) => {
		...
		if (!enableAnimations || !backgroundEnabled) return;
		...
		if (
			...
			&& backgroundEnabled
			&& enableAnimations
			&& (performanceLevel !== 'low' || songPaletteTransitionProgress < 1.0)
		) {
	...
	const startAnimation = () => {
		if (!backgroundEnabled || !enableAnimations) {
			return;
		}
	...
	const updateFromTrack = async (state: PlayerStateShape | null = null) => {
		if (performanceLevel === 'low' || !backgroundEnabled || !enableAnimations) {
	...
	onMount(() => {
		...
		const unsubPerf = effectivePerformanceLevel.subscribe((level) => {
			...
			backgroundEnabled = level !== 'low' && !isIOSDevice;
		...
		const unsubAnim = animationsEnabled.subscribe((enabled) => {
			...
			if (!enableAnimations) {
				stopAnimation();
			} else if (backgroundEnabled && canvasVisible) {
				startAnimation();
			}
		});
		...
		if (currentState.currentTrack) {
			...
			const coverUrl = resolveArtworkUrl(snapshot.currentTrack);
			backgroundEnabled = performanceLevel !== 'low' && !isIOSDevice;
			lastProcessedCover = backgroundEnabled ? coverUrl : null;
...
<div class="webgl-background" aria-hidden="true" data-performance={performanceLevel}>
	{#if (!currentTargetMasterPalette.length && !isPlaying) || performanceLevel === 'low' || !backgroundEnabled}
...
<style>
	...
	.default-background {
		...
		inset: 0;
		background: 
			radial-gradient(ellipse at 20% 30%, rgba(239, 68, 68, 0.15) 0%, transparent 50%),
			radial-gradient(ellipse at 80% 70%, rgba(99, 102, 241, 0.12) 0%, transparent 50%),
			radial-gradient(ellipse at 50% 50%, rgba(139, 92, 246, 0.08) 0%, transparent 60%),
			linear-gradient(135deg, #0a0507 0%, #14070c 55%, #240d14 100%);
