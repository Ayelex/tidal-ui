...
export function getExtensionForQuality(_quality: AudioQuality, _convertAacToMp3 = false): string {
	return 'flac';
}
...
export async function buildTrackLinksCsv(tracks: Track[], quality: AudioQuality): Promise<string> {
	...
	const rows: string[][] = [];
	const downloadQuality: AudioQuality = quality === 'LOSSLESS' ? quality : 'LOSSLESS';

	for (const [index, track] of tracks.entries()) {
		const streamUrl = await losslessAPI.getTrackStreamUrl(track.id, downloadQuality);
		rows.push([
			`${index + 1}`,
			track.title ?? '',
...
async function downloadTrackWithRetry(
	...
	for (let attempt = 1; attempt <= maxAttempts; attempt++) {
		try {
			...
			console.log(`[Track Download] OK Success: "${trackTitle}" (${(blob.size / 1024 / 1024).toFixed(2)} MB)${attempt > 1 ? ` - succeeded on attempt ${attempt}` : ''}`);
			return { success: true, blob };
		} catch (error) {
			const errorObj = error instanceof Error ? error : new Error(String(error));
			console.warn(
				`[Track Download] X Attempt ${attempt}/${maxAttempts} failed for "${trackTitle}": ${errorObj.message}`
			);
			...
			if (attempt < maxAttempts) {
				...
				console.error(
					`[Track Download] XXX All ${maxAttempts} attempts failed for "${trackTitle}" - giving up`
				);
...
export async function downloadAlbum(
	...
	console.log(`[Album Download] Tracks: ${total} | Quality: ${downloadQuality} | Mode: ${mode}`);
	console.log(`[Album Download] ------------------------------------------------`);
	...
	if (useCsv) {
		...
		const csvContent = await buildTrackLinksCsv(tracks, downloadQuality);
	...
	if (shouldZip) {
		...
		console.log(`[Album Download] OK Success: ${successCount}/${total} tracks | ZIP size: ${(zipBlob.size / 1024 / 1024).toFixed(2)} MB`);
		if (failedTracks.length > 0) {
			console.log(`[Album Download] X Failed: ${failedTracks.length} track(s) - see _DOWNLOAD_ERRORS.txt in ZIP`);
		}
		console.log(`[Album Download] ------------------------------------------------`);
	...
	console.log(`[Album Download] OK Success: ${successCount}/${total} tracks`);
	if (failedCount > 0) {
		console.log(`[Album Download] X Failed: ${failedCount} track(s)`);
	}
	console.log(`[Album Download] ------------------------------------------------`);
}
