<script lang="ts">
	...
	import { ArrowLeft, Play, User, Clock, LoaderCircle, Bookmark } from 'lucide-svelte';
	...
	const isSaved = $derived(
		$libraryStore.savedPlaylists.some((item) => item.uuid === playlist?.uuid)
	);
	...
	function toggleSavePlaylist() {
		if (!playlist) return;
		libraryStore.toggleSavedPlaylist(playlist);
	}
