<script lang="ts">
	...
	import { LoaderCircle, Play, ArrowLeft, Disc, User, Clock, Download, X, Check, Heart } from 'lucide-svelte';
...
const isLiked = $derived(
	$libraryStore.likedTracks.some((item) => item.id === (track?.id ?? -1))
);
	...
	function toggleLike() {
		if (!track) return;
		libraryStore.toggleLikedTrack(track);
	}
</script>
...
{#if isLoading}
	<div class="flex items-center justify-center py-24">
		<LoaderCircle class="h-16 w-16 animate-spin text-rose-500" />
	</div>
{:else if error}
			...
			<div class="flex flex-1 flex-col justify-end">
				...
				<div class="mb-6 space-y-2">
					<div class="flex items-center gap-2 text-xl text-gray-300">
						<User size={20} />
						<a href={`/artist/${track.artist.id}`} class="hover:text-rose-400 hover:underline">
							{formatArtists(track.artists)}
						</a>
					</div>
					<div class="flex items-center gap-2 text-lg text-gray-400">
						<Disc size={20} />
						<a href={`/album/${track.album.id}`} class="hover:text-rose-400 hover:underline">
							{track.album.title}
						</a>
					</div>
