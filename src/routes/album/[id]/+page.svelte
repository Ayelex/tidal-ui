<script lang="ts">
	...
	const isSaved = $derived($libraryStore.savedAlbums.some((item) => item.id === album?.id));
	...
	function handlePlayAll() {
		if (tracks.length > 0) {
			playerStore.playQueue(tracks, 0);
		}
	}
	...
	function toggleSaveAlbum() {
		if (!album) return;
		libraryStore.toggleSavedAlbum(album);
	}
			...
			<div class="flex flex-1 flex-col justify-end">
				...
				<div class="mb-6 flex flex-wrap items-center gap-4 text-sm text-gray-400">
					...
					{#if album.audioQuality}
						<div class="rounded bg-rose-900/30 px-2 py-1 text-xs font-semibold text-rose-400">
							{album.audioQuality}
						</div>
					{/if}
					-->
