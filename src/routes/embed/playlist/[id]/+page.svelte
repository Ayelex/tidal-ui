<script lang="ts">
	...
	const nowPlayingTrack = $derived($playerStore.currentTrack);
	const nowPlayingCoverUrl = $derived.by(() => {
		const track = $playerStore.currentTrack;
		if (!track) return '';
		return isSonglinkTrack(track)
			? track.thumbnailUrl
			: losslessAPI.getCoverUrl(track.album.cover, '80');
	});
	...
	onMount(async () => {
		try {
            ...
            window.umami?.track('embed_loaded', { host, type: 'playlist' });
        } catch {}
		...
		try {
			...
			if (typeof window !== 'undefined' && window.umami) {
				try {
					const referrer = document.referrer;
					const host = referrer ? new URL(referrer).hostname : 'direct';
					window.umami.track('embed_error', { 
						host, 
						error, 
						version: APP_VERSION,
						type: 'playlist',
...
<div class="embed-card">
    ...
    {:else if playlist}
        ...
        {:else}
            <div class="background" style="background-color: #1b0b12"></div>
        {/if}
