...
import { ArrowLeft, User, Download, LoaderCircle, Heart } from 'lucide-svelte';
...
const isLiked = $derived($libraryStore.likedArtists.some((item) => item.id === artist?.id));
...
function toggleFavoriteArtist() {
	if (!artist) return;
	libraryStore.toggleLikedArtist(artist);
}
</script>

<svelte:head>
	<title>{artist?.name || 'Artist'} - BiniLossless</title>
</svelte:head>

{#if isLoading}
	<div class="flex items-center justify-center py-24">
		<LoaderCircle size={16} class="h-16 w-16 animate-spin text-rose-500" />
	</div>
{:else if error}
	...
	<div class="space-y-6 pb-32 lg:pb-40">
				...
				<div class="mb-6 flex flex-wrap items-center gap-4">
					...
					{#if artist.artistTypes && artist.artistTypes.length > 0}
						{#each artist.artistTypes as type}
							<div
								class="rounded-full bg-rose-900/30 px-3 py-1 text-xs font-semibold text-rose-400"
							>
								{type}
							</div>
						{/each}
				...
				<div class="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
					...
					<div class="flex items-center gap-2">
						<button
							...
							type="button"
							class="inline-flex items-center gap-2 rounded-full border border-rose-600 bg-rose-600/10 px-4 py-2 text-sm font-semibold text-rose-100 transition-colors hover:bg-rose-600/20 disabled:cursor-not-allowed disabled:opacity-60"
				...
				{#if discography.length > 0}
					<div class="mt-6 grid gap-4 sm:grid-cols-3 lg:grid-cols-4 2xl:grid-cols-5">
						{#each discography as album (album.id)}
							<div
								class="group relative flex h-full flex-col rounded-xl border border-gray-800 bg-gray-900/40 p-4 text-center transition-colors hover:border-rose-700 hover:bg-gray-900"
							>
								<button
								...
								<a
									href={`/album/${album.id}`}
									class="flex flex-1 flex-col items-center gap-4 rounded-lg text-center focus:outline-none focus-visible:ring-2 focus-visible:ring-rose-500 focus-visible:ring-offset-2 focus-visible:ring-offset-neutral-900"
								>
									<div
									...
									<div class="w-full">
										<h3 class="truncate text-balance text-lg font-semibold text-white group-hover:text-rose-400">
											{album.title}
										</h3>
		...
		{#if artist.url}
			<a
				...
				class="inline-block text-sm text-rose-400 transition-colors hover:text-rose-300"
			>
				View profile ->
			</a>
		{/if}
